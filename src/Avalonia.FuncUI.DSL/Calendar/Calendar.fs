...
module Calendar =
    ...
    type Calendar with
        ...
        static member firstDayOfWeek<'t when 't :> Calendar>(value: DayOfWeek) : IAttr<'t> =
            AttrBuilder.CreateProperty<'t, DayOfWeek>(Calendar.FirstDayOfWeekProperty, value, ValueNone)
        
        static member isTodayHighlighted<'t when 't :> Calendar>(value: bool) : IAttr<'t> =
            AttrBuilder.CreateProperty<'t, bool>(Calendar.IsTodayHighlightedProperty , value, ValueNone)

        static member headerBackground<'t when 't :> Calendar>(value: IBrush) : IAttr<'t> =
            AttrBuilder.CreateProperty<'t, IBrush>(Calendar.HeaderBackgroundProperty, value, ValueNone)
        ...
        static member displayMode<'t when 't :> Calendar>(value: CalendarMode) : IAttr<'t> =
            AttrBuilder.CreateProperty<'t, CalendarMode>(Calendar.DisplayModeProperty, value, ValueNone)

        static member onDisplayModeChanged<'t when 't :> Calendar>(func: CalendarMode -> unit, ?subPatchOptions) : IAttr<'t> =
            AttrBuilder.CreateSubscription<'t, CalendarMode>(Calendar.DisplayModeProperty, func, ?subPatchOptions = subPatchOptions)

        static member selectionMode<'t when 't :> Calendar>(value: CalendarSelectionMode) : IAttr<'t> =
            AttrBuilder.CreateProperty<'t, CalendarSelectionMode>(Calendar.SelectionModeProperty, value, ValueNone)

        static member onSelectionModeChanged<'t when 't :> Calendar>(func: CalendarSelectionMode  -> unit, ?subPatchOptions) : IAttr<'t> =
            AttrBuilder.CreateSubscription<'t, CalendarSelectionMode >(Calendar.SelectionModeProperty, func, ?subPatchOptions = subPatchOptions)

        static member selectedDate<'t when 't :> Calendar>(value: DateTime Nullable) : IAttr<'t> =
            AttrBuilder.CreateProperty<'t, DateTime Nullable>(Calendar.SelectedDateProperty, value, ValueNone)
        ...
        static member onSelectedDateChanged<'t when 't :> Calendar>(func: Nullable<DateTime> -> unit, ?subPatchOptions) : IAttr<'t> =
            AttrBuilder.CreateSubscription<'t, DateTime Nullable>(Calendar.SelectedDateProperty, func, ?subPatchOptions = subPatchOptions)

        static member displayDate<'t when 't :> Calendar>(value: DateTime) : IAttr<'t> =
            AttrBuilder.CreateProperty<'t, DateTime>(Calendar.DisplayDateProperty, value, ValueNone)

        static member displayDateStart<'t when 't :> Calendar>(value: DateTime Nullable) : IAttr<'t> =
            AttrBuilder.CreateProperty<'t, DateTime Nullable>(Calendar.DisplayDateStartProperty, value, ValueNone)
        ...
        static member displayDateEnd<'t when 't :> Calendar>(value: DateTime Nullable) : IAttr<'t> =
            AttrBuilder.CreateProperty<'t, DateTime Nullable>(Calendar.DisplayDateEndProperty, value, ValueNone)
