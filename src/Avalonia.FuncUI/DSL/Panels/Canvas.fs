...
module Canvas =
    ...
    type Control with
        static member left<'t when 't :> Control>(value: double) : Attr<'t> =
            AttrBuilder<'t>.CreateProperty<double>(Canvas.LeftProperty, value, ValueNone)

        static member top<'t when 't :> Control>(value: double) : Attr<'t> =
            AttrBuilder<'t>.CreateProperty<double>(Canvas.TopProperty, value, ValueNone)

        static member right<'t when 't :> Control>(value: double) : Attr<'t> =
            AttrBuilder<'t>.CreateProperty<double>(Canvas.RightProperty, value, ValueNone)

        static member bottom<'t when 't :> Control>(value: double) : Attr<'t> =
