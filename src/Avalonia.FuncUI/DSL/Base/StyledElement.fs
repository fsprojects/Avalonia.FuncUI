...
module StyledElement =
    ...
    type StyledElement with
        static member dataContext<'t when 't :> StyledElement>(dataContext: obj) : Attr<'t> =
            AttrBuilder<'t>.CreateProperty<obj>(StyledElement.DataContextProperty, dataContext, ValueNone)

        static member name<'t when 't :> StyledElement>(name: string) : Attr<'t> =
            AttrBuilder<'t>.CreateProperty<string>(StyledElement.NameProperty, name, ValueNone)

        static member templatedParent<'t when 't :> StyledElement>(template: ITemplatedControl) : Attr<'t> =
            AttrBuilder<'t>.CreateProperty<ITemplatedControl>(StyledElement.TemplatedParentProperty, template, ValueNone)

        static member classes<'t when 't :> StyledElement>(value: Classes) : Attr<'t> =
        ...
        static member classes<'t when 't :> StyledElement>(classes: string list) : Attr<'t> =
            classes |> Classes |> StyledElement.classes

        /// Use 'classes' instead when possible.
        static member styles<'t when 't :> StyledElement>(value: Styles) : Attr<'t> =
        ...
        static member resources<'t when 't :> StyledElement>(value: IResourceDictionary) : Attr<'t> =
